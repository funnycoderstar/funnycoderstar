...
```
需要给每项的对象加一个值isEdit,来控制当前项是否为编辑的状态,刚开始代码如下
```js
initData(model) {
    for (let i = 0; i < model.length; i++) {       
        model[i].isEdit = false;
    }
    return model;
 }
```
之后调用`this.list[index].isEdit  = !this.list[index].isEdit`发现视图不更新,在updated函数中打印,发现根本没触发updated函数,改成下面的写法就可以了
```js
initData(model) {
    for (let i = 0; i < model.length; i++) {       
        model[i] = Object.assign({}, model[i], {
              isEdit: false,
        });
     }
 }
...
上述这些问题从[vue官方文档](https://vuejs.org/)中均能找到答案,当然想要更深入的理解为什么,还需要从[vue源码分析](https://github.com/vuejs/vue)入手;下一篇文章打算从源码入手去解释这些问题,理解vue整体的程序设计;
